...
certain number of words "WUB" before the first word of the song (the number
may be zero), after the last word (the number may be zero), and between words
...
def test_song_decoder(test, result):
    assert song_decoder(test) == result


def song_decoder(song: str) -> str:
    all_words = (word for word in song.split("WUB") if word != '')
